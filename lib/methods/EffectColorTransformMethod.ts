...
export class _Shader_EffectColorTransformMethod extends _Shader_MethodBase {
	...
	public _getFragmentCode(targetReg: ShaderRegisterElement, registerCache: ShaderRegisterCache, sharedRegisters: ShaderRegisterData): string {
		let code: string = '';
		const colorMultReg: ShaderRegisterElement = registerCache.getFreeFragmentConstant();
		const colorOffsReg: ShaderRegisterElement = registerCache.getFreeFragmentConstant();
	...
	public _activate(): void {
		if (this._invalid) {
			const index: number = this._colorTransformIndex;
			const data: Float32Array = this._shader.fragmentConstantData;
			const rawData: Float32Array = this._method.colorTransform._rawData;

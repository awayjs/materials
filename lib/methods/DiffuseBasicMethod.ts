...
export class DiffuseBasicMethod extends MethodBase {
	...
	public copyFrom(method: MethodBase): void {
		const diff: DiffuseBasicMethod = <DiffuseBasicMethod> method;
...
export class _Shader_DiffuseBasicMethod extends _Shader_MethodBase implements _IShader_Method {
	...
	public _getFragmentCode(targetReg: ShaderRegisterElement, registerCache: ShaderRegisterCache, sharedRegisters: ShaderRegisterData): string {
		...
		const ambientColorRegister: ShaderRegisterElement = registerCache.getFreeFragmentConstant();
		...
		if (this._texture) {
			...
			const diffuseColorRegister: ShaderRegisterElement = registerCache.getFreeFragmentConstant();
	...
	public _activate(): void {
		if (this._texture) {
			...
			const index: number = this._diffuseColorRegister;
	...
	public _setRenderState(renderState: _Render_RenderableBase): void {
		...
		const index: number = this._ambientColorRegister;
		const data: Float32Array = this._shader.fragmentConstantData;

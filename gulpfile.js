...
gulp.task('package-min', ['package'], function(callback){
    return gulp.src('./build/awayjs-methodmaterials.js')
...
function browserifyShare(callback) {
    ...
    b.plugin('tsify', {target:'ES5', sourceRoot:'../', noExternalResolve: true, declarationFiles: './node_modules/awayjs-**/build/*.d.ts', declarationOutput: './build/awayjs-methodmaterials.d.ts'});
    ...
    glob('./lib/**/*.ts', {}, function (error, files) {
        ...
        b.bundle()
            .pipe(exorcist('./build/awayjs-methodmaterials.js.map'))
            .pipe(source('awayjs-methodmaterials.js'))
            .pipe(gulp.dest('./build'))
            .on('end', callback);
    });
